...
func runCj(cjDir *string) {
	...
	if err == nil {
		...
		// no need to rerun user.Current()
		scheduleCj(cjDir, &curUser.Username)
	} else {
		// log exit status
...
func scheduleCj(cjDir *string, curUser *string) {
	...
	if err != nil {
		log.Printf("awecron error (%s) {%s}: problem reading cronjob config file cjDir/cfg and saving as cronjob config data cjCfgData", *curUser, path.Base(*cjDir))
		return
	}
	// conversion
	cjCfg, err := strconv.Atoi(strings.TrimSpace(string(cjCfgData)))
	if err != nil {
		log.Printf("awecron error (%s) {%s}: problem converting cronjob config data cjCfgData into cronjob config integer cjCfg", *curUser, path.Base(*cjDir))
		return
	}
	// make sure its greater than zero
	if cjCfg <= 0 {
		log.Printf("awecron error (%s) {%s}: cronjob config cjCfg should be greater than zero", *curUser, path.Base(*cjDir))
		return
	}
	...
	if err != nil {
		log.Fatalf("awecron fatal (%s) {%s}: problem creating cjDir/tmr file", *curUser, path.Base(*cjDir))
	}
	// closing cjTmr file
	err = cjTmr.Close()
	if err != nil {
		log.Fatalf("awecron fatal (%s) {%s}: problem closing tmr file cjTmr", *curUser, path.Base(*cjDir))
	}
	...
	if err != nil {
		log.Fatalf("awecron fatal (%s) {%s}: problem setting last modification time of tmr file", *curUser, path.Base(*cjDir))
	}
}
